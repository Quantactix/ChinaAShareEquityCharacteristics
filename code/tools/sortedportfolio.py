...
class port:
	...
	def bisort(self):
		...
		self.df['bucket_size'] = \
			np.where(self.df['rank_size']<=0,
			'size1',
			'size2')
		self.df['port_bi_'+self.char] = self.df['bucket_size']+self.df['bucket_char']
		del self.df['bucket_char'], self.df['bucket_size']

	def get_port_attribute(self):

		self.uni_count = self.df.groupby(['date','port_uni_'+self.char]).count()['ret'].unstack()
		if not self.char=='size':
	...
	def long_short_portfolio(self):
		...
		if self.uni_ret[str(self.n-1)].mean() >= self.uni_ret['0'].mean():
			...
			if not self.char=='size':
				self.bi_ls = 1/2*(self.bi_ret['size1'+self.char+'3']+self.bi_ret['size2'+self.char+'3']) \
					    - 1/2*(self.bi_ret['size1'+self.char+'1']+self.bi_ret['size2'+self.char+'1'])
		else:
			self.ls_sign = 'Long Low'
			self.uni_ls = self.uni_ret['0'] - self.uni_ret[str(self.n-1)]
			if not self.char=='size':
				self.bi_ls = 1/2*(self.bi_ret['size1'+self.char+'1']+self.bi_ret['size2'+self.char+'1']) \
					    - 1/2*(self.bi_ret['size1'+self.char+'3']+self.bi_ret['size2'+self.char+'3'])
		print(self.ls_sign)
		# Sharpe ratio
